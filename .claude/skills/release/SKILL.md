...
3. goreleaser builds cross-platform archives, creates the GitHub Release, and updates the Homebrew tap at `thomas0124/homebrew-tap`.
...
8. **Verify the Homebrew tap update.** Fetch the tap Formula and confirm the version bumped:
   - `gh api repos/thomas0124/homebrew-tap/contents/ralph.rb --jq '.content' | base64 -d | grep -E 'version|url'`
...
9. **Report completion.** Show the user:
   ...
   - The command to install the new build: `brew update && brew upgrade ralph` (or `brew install thomas0124/tap/ralph` for fresh installs)
