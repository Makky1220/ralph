...
// mergeEnv syncs the top-level "env" object.
...
// mergeOwnedArray 3-way merges a single owned array path.
...
// mergeHooks syncs every owned event array under the top-level "hooks" object.
...
// present and is now an empty object.
...
// setOwnedArray writes result at path under merged.
...
// parseSettingsDoc decodes data into an ordered JSON object value.
...
// preferred's order first, then any secondary-only keys in secondary's order.
